...
def er_mse_loss(input, target, w):
    loss = F.mse_loss(input, target, reduction="none")
    num_dims = len(loss.shape)
    if num_dims > 1:
        loss = loss.mean(dim=tuple(range(1, num_dims)))
    return torch.mean(loss * w)


def hcl_loss(fstudent, fteacher, w_EA):
    loss_all = 0.0
    for fs, ft in zip(fstudent, fteacher):
        n, c, h, w = fs.shape
        loss = er_mse_loss(fs, ft, w_EA)
        cnt = 1.0
        tot = 1.0
        for l in [4, 2, 1]:
            if l >= h:
            ...
            cnt /= 2.0
            loss += er_mse_loss(tmpfs, tmpft, w_EA) * cnt
            tot += cnt
        loss = loss / tot
...
class ReviewKD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if self.EA:
            w_EA = get_w(logits_student, logits_teacher, self.H_ub, self.t).detach()
        else:
            w_EA = torch.ones_like(logits_student).detach()
        loss_reviewkd = (
            self.reviewkd_loss_weight
            * min(kwargs["epoch"] / self.warmup_epochs, 1.0)
            * hcl_loss(results, features_teacher, w_EA)
        )
        losses_dict = {
            "loss_ce": loss_ce,
